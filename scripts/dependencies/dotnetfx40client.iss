...
[CustomMessages]
dotnetfx40client_title=Microsoft .NET Framework 4.0 Client

[Code]
const
	dotnetfx40client_size_min=3;
	dotnetfx40client_size_max=197;
...
begin
	if (not netfxinstalled(NetFx40Client, '')) then begin
		MIDI_AddProduct('dotNetFx40_Client_setup.exe',
			'/lcid ' + CustomMessage('MIDI_lcid') + ' /passive /norestart',
			CustomMessage('dotnetfx40client_title'),
			MIDI_GetSizeString (dotnetfx40client_size_min, dotnetfx40client_size_min, 0) + ' - ' + MIDI_GetSizeString (dotnetfx40client_size_max, dotnetfx40client_size_max, 0),
			dotnetfx40client_url,
			false, false);
	end else begin	
		MIDI_AddInstalledProduct('dotNetFx40_Client_setup.exe',
			CustomMessage('dotnetfx40client_title'),
			MIDI_GetSizeString (dotnetfx40client_size_min, dotnetfx40client_size_min, 0) + ' - ' + MIDI_GetSizeString (dotnetfx40client_size_max, dotnetfx40client_size_max, 0),
			dotnetfx40client_url,
			false, false);
